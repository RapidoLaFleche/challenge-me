...
class _AdminPostsScreenState extends State<AdminPostsScreen> {
  ...
  Future<void> _updateStatus(String postId, String status) async {
    try {
      final response = await supabase.from('posts').update({'status': status}).eq('id', postId);
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Post mis à jour !')),
      );
      ...
      body: _isLoading
          ...
          : ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                return Card(
                  color: post['status'] == 'pending' ? const Color.fromARGB(255, 46, 46, 46) : Colors.green[800],
                              ...
                              children: [
                                if (post['status'] != 'approved')
                                  IconButton(
                                    ...
                                    onPressed: () => _updateStatus(post['id'].toString(), 'approved'),
                                  ),
                                if (post['status'] != 'pending')
                                  IconButton(
                                    icon: const Icon(Icons.pause, color: Colors.white),
                                    onPressed: () => _updateStatus(post['id'].toString(), 'pending'),
                                  ),
                              ],
                            ),
                          ],
